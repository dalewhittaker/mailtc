...
int imapcram_get_messages(void *pdata)
{
	mail_details *paccount= (mail_details *)pdata;
	return(check_cramimap_mail(paccount, mtc_dir));
}

/*this is called when the plugin is first loaded. it is here that we set globals passed from the core app*/
int imapcram_load(const char *cfgdir, void *plog, unsigned int flags)
{
...
int imapcram_clicked(void *pdata)
{
	mail_details *paccount= (mail_details *)pdata;
